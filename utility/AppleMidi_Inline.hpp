...
/*! \brief Generate and send a MIDI message from the values given.
 ...
 from your code, at your own risks.
 */
void AppleMidi_Class::internalSend(Session_t* session,
			        			 MidiType inType,
                                 DataByte inData1,
                                 DataByte inData2,
                                 Channel inChannel)
{
    ...
    if (inType <= PitchBend)  // Channel messages
    {
		...
		RtpMidi_t rtpMidi;
/*		int i = rtpHeader(session);

		// Length
		_packetBuffer[i++] = 0x03; // B-Flag: Short header, ....
 ...
 //       mSerial.write(status);
        ...
        //mSerial.write(inData1);
 		_packetBuffer[i++] = inData1;
       if (inType != ProgramChange && inType != AfterTouchChannel)
            _packetBuffer[i++] = inData2;

		writeBuffer(_packetBuffer, i);
*/
        return;
    }
 ...
 */
void AppleMidi_Class::sendNoteOn(DataByte inNoteNumber,
                                 DataByte inVelocity,
                                 Channel inChannel)
{ 
...
/*! \brief Send a Note Off message
 ...
 http://www.phys.unsw.edu.au/jw/notes.html
 */
void AppleMidi_Class::sendNoteOff(DataByte inNoteNumber,
                                  DataByte inVelocity,
                                  Channel inChannel)
{
...
/*! \brief Send a Program Change message 
 ...
 \param inChannel       The channel on which the message will be sent (1 to 16).
 */
void AppleMidi_Class::sendProgramChange(DataByte inProgramNumber,
                                        Channel inChannel)
{
 ...
 */
void AppleMidi_Class::sendControlChange(DataByte inControlNumber,
...
/*! \brief Send a Polyphonic AfterTouch message (applies to a specified note)
 ...
 \param inChannel     The channel on which the message will be sent (1 to 16). 
 */
void AppleMidi_Class::sendPolyPressure(DataByte inNoteNumber,
                                       DataByte inPressure,
                                       Channel inChannel)
{
...
/*! \brief Send a MonoPhonic AfterTouch message (applies to all notes)
 ...
 \param inChannel     The channel on which the message will be sent (1 to 16). 
 */
void AppleMidi_Class::sendAfterTouch(DataByte inPressure,
                                     Channel inChannel)
{
...
/*! \brief Send a Pitch Bend message using a signed integer value.
 ...
 \param inChannel     The channel on which the message will be sent (1 to 16).
 */
void AppleMidi_Class::sendPitchBend(int inPitchValue,
                                    Channel inChannel)
{
...
/*! \brief Send a Pitch Bend message using a floating point value.
 ...
 \param inChannel     The channel on which the message will be sent (1 to 16).
 */
void AppleMidi_Class::sendPitchBend(double inPitchValue,
                                    Channel inChannel)
{
 ...
 */
void AppleMidi_Class::sendTimeCodeQuarterFrame(DataByte inTypeNibble, 
 ...
 */
bool AppleMidi_Class::read(Channel inChannel)
{
    if (inChannel >= MIDI_CHANNEL_OFF)
...
#if (APPLEMIDI_BUILD_OUTPUT && MIDI_BUILD_THRU)
...
bool AppleMidi_Class::inputFilter(Channel inChannel)
{
...
void AppleMidi_Class::resetInput()
{
    mPendingMessageIndex = 0;
    mPendingMessageExpectedLenght = 0;
    mRunningStatus_RX = InvalidType;
 ...
 */
MidiType AppleMidi_Class::getType() const
{
 ...
 */
const byte* AppleMidi_Class::getSysExArray() const
{ 
 ...
 */
unsigned int AppleMidi_Class::getSysExArrayLength() const
{
...
bool AppleMidi_Class::check() const
{ 
...
Channel AppleMidi_Class::getInputChannel() const 
{
 ...
 */
void AppleMidi_Class::setInputChannel(Channel inChannel)
{ 
 ...
 */
MidiType AppleMidi_Class::getTypeFromStatusByte(byte inStatus) 
{
    if ((inStatus  < 0x80) ||
        (inStatus == 0xF4) ||
        (inStatus == 0xF5) ||
...
bool AppleMidi_Class::isChannelMessage(MidiType inType)
{
...
void AppleMidi_Class::setHandleConnected(void (*fptr)(char*))    { mConnectedCallback    = fptr; }
void AppleMidi_Class::setHandleDisconnected(void (*fptr)())      { mDisconnectedCallback = fptr; }

void AppleMidi_Class::setHandleNoteOff(void (*fptr)(byte channel, byte note, byte velocity))          { mNoteOffCallback              = fptr; }
void AppleMidi_Class::setHandleNoteOn(void (*fptr)(byte channel, byte note, byte velocity))           { mNoteOnCallback               = fptr; }
void AppleMidi_Class::setHandleAfterTouchPoly(void (*fptr)(byte channel, byte note, byte pressure))   { mAfterTouchPolyCallback       = fptr; }
void AppleMidi_Class::setHandleControlChange(void (*fptr)(byte channel, byte number, byte value))     { mControlChangeCallback        = fptr; }
void AppleMidi_Class::setHandleProgramChange(void (*fptr)(byte channel, byte number))                 { mProgramChangeCallback        = fptr; }
void AppleMidi_Class::setHandleAfterTouchChannel(void (*fptr)(byte channel, byte pressure))           { mAfterTouchChannelCallback    = fptr; }
void AppleMidi_Class::setHandlePitchBend(void (*fptr)(byte channel, int bend))                        { mPitchBendCallback            = fptr; }
void AppleMidi_Class::setHandleSystemExclusive(void (*fptr)(byte* array, byte size))                  { mSystemExclusiveCallback      = fptr; }
void AppleMidi_Class::setHandleTimeCodeQuarterFrame(void (*fptr)(byte data))                          { mTimeCodeQuarterFrameCallback = fptr; }
void AppleMidi_Class::setHandleSongPosition(void (*fptr)(unsigned int beats))                         { mSongPositionCallback         = fptr; }
void AppleMidi_Class::setHandleSongSelect(void (*fptr)(byte songnumber))                              { mSongSelectCallback           = fptr; }
void AppleMidi_Class::setHandleTuneRequest(void (*fptr)(void))                                        { mTuneRequestCallback          = fptr; }
void AppleMidi_Class::setHandleClock(void (*fptr)(void))                                              { mClockCallback                = fptr; }
void AppleMidi_Class::setHandleStart(void (*fptr)(void))                                              { mStartCallback                = fptr; }
void AppleMidi_Class::setHandleContinue(void (*fptr)(void))                                           { mContinueCallback             = fptr; }
void AppleMidi_Class::setHandleStop(void (*fptr)(void))                                               { mStopCallback                 = fptr; }
void AppleMidi_Class::setHandleActiveSensing(void (*fptr)(void))                                      { mActiveSensingCallback        = fptr; }
void AppleMidi_Class::setHandleSystemReset(void (*fptr)(void))                                        { mSystemResetCallback          = fptr; }
 ...
 */ 
void AppleMidi_Class::disconnectCallbackFromType(MidiType inType)
{
    switch (inType)
    {
...
void AppleMidi_Class::launchCallback()
{
